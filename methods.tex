...
\subsubsection*{** tags are considered per user while images are not}
Evidence of tags is provided by Flickr users. According to their observation and opinion, tags are associated with  images. Tags provided by the same users are likely to be biased in the same way. And we assume users make mistakes with a certain probability. On the other hand, images are less likely to be biased according to users. It's the vision classifier not the photographer determines an image is with some ecology phenomenon or not. And this vision model has its own error rate. (?? I didn't explain the fixed error rate of gps and temporal information. I have question here.)
