...
~\cite{Geo-Clustering of Images With Missing GeoTags} estimates where images are taken for those missing geo-tags. They are optimizing a graph clustering problem. Attributes in their graph include textual tags, timestamps and vision content. It's inspired by an earlier work
~\cite{Mapping the world’s photos}.
~\cite{Geo-Location Estimation of Flickr Images: Social
Web Based Enrichment} consider textual meta-data to correct geo tags. 
...
Timestamp is harder to be accurate due to the same reason of geo-tags and the time-zone problem. In paper ~\cite{Who's Time is it anyway?}, there is a detail analysis in disagreement of camera time and GPS time. They also estimate a more accurate timestamp when users taking multiple images in a short timespan.
...
\vspace{-8pt}
\xhdr{Crowd-sourcing from social media} Several recent studies have
...
\xhdr{Recognizing flowers} There are a number of papers on detecting
...
% They test their system
%on a dataset with 17 flower species.
%
%
%% %% The system is tested on 17 species and get best performance of  81.3\%.
